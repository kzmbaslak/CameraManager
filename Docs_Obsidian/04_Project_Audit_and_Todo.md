...
- Alarm operasyon akışı büyük ölçüde tamamlandı: yeni/onaylandı/çözüldü, atanan kullanıcı, not, önem seviyesi, olay kapatma nedeni, yanlış alarm işareti ve detay paneli var. Filtrelenmiş alarm listesi açık alarm/yüksek öncelik/yanlış alarm oranı/ortalama çözüm süresi metrikleriyle raporlanır ve CSV olarak dışa aktarılabilir. Yanlış alarm olarak kapatılmış insan tespitleri AI geri bildirim CSV'siyle bbox/confidence/not/çözüm nedeni/hash alanları üzerinden eğitim/threshold iyileştirme havuzuna aktarılabilir. Kalan iş zamanlanmış kurumsal rapor üretimi.
...
- Sidebar desktop'ta sabit, küçük ekran/tablet için üst bar ve overlay drawer davranışı eklendi. Desktop sidebar ikon moduna daraltılabilir ve tercih tarayıcıda kalıcı saklanır.
...
- `confirm()`/`alert()` kullanımı kaldırıldı; kamera, NVR ve kullanıcı silme akışları ortak erişilebilir `ConfirmDialog` modalını kullanıyor. Global toast standardı eklendi ve kamera/NVR/kullanıcı/alarm operasyonlarına başarı/başarısızlık bildirimleri bağlandı. Ortak API hata mesajı yardımcı fonksiyonu ile kamera, NVR, kullanıcı ve alarm hataları backend `detail` veya okunabilir fallback mesajını gösteriyor. Kamera/NVR/kullanıcı ekleme ve düzenleme formlarında alan bazlı inline validasyon; kamera tarama ve NVR keşif/toplu ekleme akışlarında satır bazlı inline validasyon eklendi.
...
3. Alarm detay deneyimi: detay drawer, snapshot preview, not/atanan kişi/çözüm nedeni, önem seviyesi, yanlış alarm işareti ve AI geri bildirim CSV export'u eklendi; sıradaki iyileştirme zamanlanmış raporlama.
