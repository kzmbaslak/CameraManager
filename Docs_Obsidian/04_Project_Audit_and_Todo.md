...
- `confirm()`/`alert()` kullanımı kaldırıldı; kamera, NVR ve kullanıcı silme akışları ortak erişilebilir `ConfirmDialog` modalını kullanıyor. Global toast standardı eklendi ve kamera/NVR/kullanıcı/alarm operasyonlarına başarı/başarısızlık bildirimleri bağlandı. Ortak API hata mesajı yardımcı fonksiyonu ile kullanıcı yönetimi, alarm aksiyonları ve NVR düzenleme hataları backend `detail` veya okunabilir fallback mesajını gösteriyor. Kalan iş daha ince alan bazlı validasyon uyarılarını inline alan seviyesine indirmek.
